...
final readonly class PlayerGame
{
    private function __construct(
        ...
        final private ?string $completionDurationLabel,
    ) {
    }
    ...
    public static function fromArray(array $row, ?string $completionDurationLabel = null): self
    {
        return new self(
            ...
            maxRarityPoints: (int) ($row['max_rarity_points'] ?? 0),
            maxInGameRarityPoints: (int) ($row['max_in_game_rarity_points'] ?? 0),
            bronze: (int) ($row['bronze'] ?? 0),
            silver: (int) ($row['silver'] ?? 0),
            gold: (int) ($row['gold'] ?? 0),
            platinum: (int) ($row['platinum'] ?? 0),
            progress: (int) ($row['progress'] ?? 0),
            lastUpdatedDate: (string) ($row['last_updated_date'] ?? ''),
