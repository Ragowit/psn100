            ...
            <div class="col-md-12">
                <table class="table table-responsive table-striped">
                    ...
                    if (isset($_GET["country"])) {
                        ...
                        $query = $database->prepare("SELECT * FROM player WHERE `rank` != 0 AND country = :country ORDER BY `rank` LIMIT :offset, :limit");
                        $query->bindParam(":country", $country, PDO::PARAM_STR);
                    } else {
                        $query = $database->prepare("SELECT * FROM player WHERE `rank` != 0 ORDER BY `rank` LIMIT :offset, :limit");
                    }
