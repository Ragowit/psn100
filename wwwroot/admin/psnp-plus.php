...
if (is_string($flashSuccess)) {
    $trimmed = trim($flashSuccess);
    if ($trimmed !== '') {
        $successMessages[] = $trimmed;
    }
}

$flashError = $_GET['error'] ?? null;
if (is_string($flashError)) {
    $trimmed = trim($flashError);
    if ($trimmed !== '') {
        $errorMessages[] = $trimmed;
    }
}
...
<html lang="en" data-bs-theme="dark">
    ...
    <body>
        <div class="p-4">
            ...
            <?php if ($psnpPlusReport !== null && $errorMessages === []) { ?>
