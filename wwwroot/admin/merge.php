...
if (ctype_digit(strval($_POST["parent"])) && ctype_digit(strval($_POST["child"]))) {
    ...
    $players = $database->prepare("SELECT account_id, last_updated_date FROM trophy_title_player
    ...
    while ($player = $players->fetch()) {
        // Copy the trophies
        $childTrophies = $database->prepare("SELECT np_communication_id, group_id, order_id, earned_date FROM trophy_earned
            WHERE np_communication_id = (SELECT np_communication_id FROM trophy_title WHERE id = :game_id) AND account_id = :account_id");
        ...
        // trophy_group_player
        $groups = $database->prepare("SELECT DISTINCT parent_np_communication_id, parent_group_id FROM trophy_merge
                WHERE child_np_communication_id = (SELECT np_communication_id FROM trophy_title WHERE id = :game_id)");
        ...
        // trophy_title_player
        $titles = $database->prepare("SELECT DISTINCT parent_np_communication_id FROM trophy_merge
                WHERE child_np_communication_id = (SELECT np_communication_id FROM trophy_title WHERE id = :game_id)");
