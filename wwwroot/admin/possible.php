...
<html lang="en">
    ...
    <body>
        ...
        $sql .= " OR (te.np_communication_id = 'NPWR12464_00' AND te.order_id = 4)
            OR (te.np_communication_id = 'NPWR12464_00' AND te.order_id = 9)
        ...
        $sql .= " OR (te.np_communication_id = 'NPWR19903_00' AND te.order_id = 0 AND te.earned_date < '2020-09-01')
            OR (te.np_communication_id = 'NPWR19903_00' AND te.order_id = 21 AND te.earned_date < '2020-09-01')
        ...
        $query = $database->prepare("SELECT account_id, online_id, TIMESTAMPDIFF(SECOND, first_trophy, second_trophy) time_difference
            FROM player p
            JOIN (SELECT earned_date AS first_trophy, account_id FROM trophy_earned WHERE np_communication_id = 'NPWR00928_00' AND order_id = 47) trophy_start USING (account_id)
            JOIN (SELECT earned_date AS second_trophy, account_id FROM trophy_earned WHERE np_communication_id = 'NPWR00928_00' AND order_id = 50) trophy_end USING (account_id)
            WHERE p.status != 1
            HAVING time_difference <= 0
            ORDER BY online_id");
        $query->execute();
